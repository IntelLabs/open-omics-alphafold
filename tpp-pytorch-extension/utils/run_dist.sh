...
function print_vars {
  for VAR in ${!CCL*} ${!I_MPI*} ${!i_mpi*} ${!PSM3*} ${!FI_*} ${!KMP_*} ${!OMP_*} ${!GOMP*} ${!ATL_*} LD_PRELOAD LD_LIBRARY_PATH ${!DLRM_*} ${!PYTORCH_*} ${!TPP_*} ${!LIBXSMM_*} ${!EMULATE_*} DATALOADER_WORKER_COUNT VIRTUAL_ENV ${!ARGS_*} $@ ; do
    if ! test -z ${!VAR} ; then
       echo "Using $VAR=${!VAR}"
    fi
  done

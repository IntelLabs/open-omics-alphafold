...
class LinearWarmupPolyDecayScheduler(LRScheduler):
    ...
    def __init__(
        ...
        mlperf_logger.log_event(
            key=mlperf_logger.constants.OPT_LR_WARMUP_STEPS,
            value=self.num_warmup_updates,
            sync=False,
        )
        mlperf_logger.log_event(
            key="opt_lamb_learning_rate_decay_poly_power", value=degree, sync=False
        )
        mlperf_logger.log_event(
            key="start_warmup_step", value=self.start_warmup_steps, sync=False
        )
